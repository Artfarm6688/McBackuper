...
"""
===============================================================
McBackuper Utility v1.1.1
Developed by Art_Farm (https://github.com/Artfarm6688)

⚠️ CRITICAL WARNING:
This script performs low-level file operations and RCON commands. 
Unauthorized modifications may lead to data corruption or server instability.
...
class BackupJob:
    ...
    def _execute_rcon(self, commands: list[str]) -> None:
        if not commands or not self.cfg.get('rcon_enabled') or self.stop_event.is_set():
            return
    ...
    def _copy_server_to_temp(self, temp_dir: Path) -> bool:
        ...
        def walk_err_handler(err: OSError) -> None:
            self.logger.error(f"Inaccessible critical directory {err.filename}: {err.strerror}")
            raise err
        ...
        try:
            ...
            self.logger.error(f"Staging copy critically failed due to IO permissions: {e}")
            return False
        except Exception as e:
            ...
            with zipfile.ZipFile(tmp_zip, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=int(self.cfg.get('compression_level', 6))) as zf:
                for root, dirs, files in os.walk(source_dir):
                    ...
                    for d in dirs:
                        rel_dir = rel_root / d
                        rel_str = str(rel_dir)
                        if rel_str not in added_dirs:
                            zf.write(root_path / d, rel_dir)
                            added_dirs.add(rel_str)

                    for file in files:
                        if self.stop_event.is_set():
                            return False
                        rel_file = rel_root / file
                        zf.write(root_path / file, rel_file)
    ...
    def _rotate_backups(self) -> None:
        if not self.cfg.get('auto_delete_old_backups') or self.stop_event.is_set():
            return
...
class BackupManager:
    def __init__(self, base_dir: Path, config_path: Path):
        ...
        self.backups_dir: Path = base_dir
...
# Path resolution:
# If you provide relative paths below (like "./backups"), they will be resolved 
# relative to this config file's directory. Absolute paths (like "/var/backups") 
# are also supported and will be used exactly as written.
global_backup_dir = "./backups"
global_temp_dir = "./tmp"
...
"""
    ...
    def _run_servers(self, servers_to_run: list[dict[str, Any]]) -> bool:
        ...
        while not_done and not self.stop_event.is_set():
            done, not_done = wait(
                not_done, 
                timeout=1.0, 
        ...
        if self.stop_event.is_set():
            self.logger.warning("Shutdown event active. Detaching and cancelling pending tasks...")
            for f in not_done:
                f.cancel()
...
def main() -> None:
    ...
    if hasattr(os, 'geteuid') and os.geteuid() == 0:
        logger.warning("WARNING: Running as root/superuser! This is not recommended and may cause file permission issues. Ensure correct permissions.")
